...
def logistic(z):
    ...
    ### YOUR CODE HERE
    logi = np.array([1/(1 + np.exp(-val)) for val in z])  # Using sigmoid on each entry and making a new array
    ### END CODE
...
class LogisticRegressionClassifier():
    ...
    def cost_grad(self, X, y, w):
        ...
        ### YOUR CODE HERE
        n = X.shape[0]    
        exponent = -y * (X @ w)  # Computation of the exponent       
        cost = np.mean(np.log(1 + np.exp(exponent)))  # Cost function from slides      
        logistic_values = logistic(exponent)  # Logistic function defined above    
        grad = -(X.T @ (y * logistic_values)) / n  # Gradient from slides
        ### END CODE
        assert grad.shape == w.shape
        return cost, grad


    def fit(self, X, y, w=None, lr=0.2, batch_size=64, epochs=10):
        """
        ...
        ### YOUR CODE HERE 
        for _ in range(epochs):  # looping for each epoch
                        
            indices = np.random.permutation(X.shape[0])  # creating random indices to find random bacthes       
                  
            for start in range(0, X.shape[0], batch_size):  # creating each batch and making gradient descent on the created batch
                end = start + batch_size
                batch_indices = indices[start:end]
                X_batch = X[batch_indices]
                y_batch = y[batch_indices]
                _, gradient = self.cost_grad(X_batch, y_batch, w)  # using the gradient and cost from method above
                direction = -gradient
                w += lr * direction
                
            cost, _ = self.cost_grad(X, y, w)  # Calculating the cost for the whole dataset
            history.append(cost)  # adding the cost to the history list
        ### END CODE
    ...
    def predict(self, X):
        ...
        probabilities = logistic(self.w @ X.T)  # Applying logistic on the data and returning a class which is most probable
        out[probabilities < 0.5] = -1
        out[probabilities >= 0.5] = 1
        ### END CODE
        return out
    ...
    def score(self, X, y):
        ...
        s = 0
        ### YOUR CODE HERE
        s = np.sum(self.predict(X) == y)  # score from slides
        s = s/X.shape[0]
        ### END CODE
        return s
